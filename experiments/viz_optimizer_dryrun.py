"""Visualize MPPI vs MPPI-CMA from quick dryrun agg data.

Uses pre-computed agg.npz from outputs/mppi_cma_quick_dryrun/.
Labels MPPI as "Spider-Origin-Optimizer".

Usage:
    cd spider/
    .venv/bin/python experiments/viz_optimizer_dryrun.py
"""
...
OUT = os.path.join(BASE, "outputs", "mppi_cma_quick_dryrun")
...
data = {}
for key in EXPS:
    path = os.path.join(OUT, f"{key}_agg.npz")
    data[key] = dict(np.load(path))
...
print(f"tick_ids={tick_ids}, n_ticks={n_ticks}, n_seeds={n_seeds}, n_iters={n_iters}")

# ── Fig 1: Convergence per focus tick (all 4 combos) ──
...
for t_idx in range(n_ticks):
    ax = axes[t_idx]
    tick = int(tick_ids[t_idx]) if tick_ids.ndim > 0 else int(tick_ids)
...
plt.suptitle("Reward Convergence: Spider-Origin-Optimizer vs MPPI-CMA (3 seeds)", fontsize=13)
plt.tight_layout()
fig.savefig(os.path.join(OUT, "fig1_convergence_all.png"), dpi=150, bbox_inches="tight")
print("Saved fig1_convergence_all.png")

# ── Fig 2: Optimizer comparison only (gain decay, side by side) ──
fig, axes = plt.subplots(1, n_ticks, figsize=(6 * n_ticks, 5), sharey=True)
if n_ticks == 1:
    axes = [axes]
...
plt.suptitle("Contact Guidance: Spider-Origin-Optimizer vs MPPI-CMA (3 seeds)", fontsize=13)
plt.tight_layout()
fig.savefig(os.path.join(OUT, "fig2_optimizer_compare.png"), dpi=150, bbox_inches="tight")
print("Saved fig2_optimizer_compare.png")

# ── Fig 3: Reward components at tick 5 (2×2 grid matching original) ──
...
# Use tick index 1 (tick 5) if available
target_tick = 5
...
plt.suptitle(f"Reward Components at Tick {actual_tick}", fontsize=13)
plt.tight_layout()
...
ax.set_xticks(x)
ax.set_xticklabels([EXPS[k]["label"] for k in EXPS], fontsize=8, rotation=15, ha="right")
