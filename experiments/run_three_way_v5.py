#!/usr/bin/env python3
"""Three-way comparison v5: Shared warmup + rew_exploit (mean trajectory reward).

v5: Uses proper Hansen (μ/μ_w, λ)-CMA-ES with evolution paths, step-size
adaptation (CSA), and rank-one + rank-μ covariance update.

Conditions:
  1. mppi           — vanilla MPPI (SPIDER original, no virtual contact distance)
  2. mppi_cma_full  — MPPI softmax weighting + full covariance adaptation
  3. cma_es_full    — Hansen's CMA-ES (evolution paths + CSA + full C update)
"""
...
def run_optimizer_from_snapshot(config, env, ref_data, rollout, ctrl_ref,
    ...
    if engine == "mppi_cma_full":
        ...
        cma_state = _init_cma_state(d, mean_init, sigma0, config.num_samples, config.device)
...
def plot_results(all_data, output_dir, full_iters):
    ...
    STYLES = {
        ...
        "cma_es_full":     {"color": "#4C72B0", "ls": "-",  "label": "CMA-ES (Hansen, full)"},
    }
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/three_way_v5")
    ...
    log(f"Three-Way v5 (Hansen CMA-ES + shared warmup + zero init + rew_exploit)")
